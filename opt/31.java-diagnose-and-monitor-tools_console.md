...
## jmap
在这种情况下，我们便可以请jmap命令出马，分析 Java 虚拟机堆中的对象。
...
这里`format=b`将使jmap导出与hprof（在 Java 9 中已被移除）、`-XX:+HeapDumpAfterFullGC`、`-XX:+HeapDumpOnOutOfMemoryError`格式一致的文件。这种格式的文件可以被其他 GUI 工具查看，具体我会在下一篇中进行演示。
...
jinfo命令可用来查看目标 Java 进程的参数，如传递给 Java 虚拟机的\-X（即输出中的 jvm\_args）、\-XX参数（即输出中的 VM Flags），以及可在 Java 层面通过System.getProperty获取的\-D参数（即输出中的 System Properties）。
...
举个例子，我们可以使用jinfo -flag +HeapDumpAfterFullGC \<PID\>命令，开启\<PID\>所指定的 Java 进程的`HeapDumpAfterFullGC`参数。
...
你还可以直接使用jcmd命令，来替代前面除了jstat之外的所有命令。具体的替换规则你可以参考下表。
...
#### [上一篇：基准测试框架JMH（下）](30.benchmarking-framework-jmh.md)
#### [下一篇：Java虚拟机监控和诊断工具（图形化界面篇）](32.java-diagnose-and-monitor-tools_gui.md)
