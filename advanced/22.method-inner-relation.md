在上一篇中，举的例子都是静态方法调用，即时编译器可以轻易地确定唯一的目标方法。
...
```
上，即便调用者的声明类型为 Add，即时编译器仍需为之添加假设。这是因为 Java 虚拟机不能保证没有重写了 apply 方法的 Add 类的子类。
...
### [上一篇：方法内联（上）](21.method-inner-relation.md)
#### [下一篇：Hotspot虚拟机intrinsic](23.hotspot-jvm-intrinsic.md)
