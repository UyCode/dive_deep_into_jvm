...
当然了，这种墨守成规的调用方式，自然是先进分子所不齿的。现在年轻人里流行的是鸭子类型（duck typing），只要是跑起来像只马的，它就是一只马，也就能够参加赛马比赛。
...
增操作还可以用来实现方法的柯里化。举个例子，有一个指向 f(x, y) 的方法句柄，我们可以通过将 x 绑定为 4，生成另一个方法句柄 g(y) = f(4, y)。在执行过程中，每当调用 g(y) 的方法句柄，它会在参数列表最前面插入一个 4，再调用指向 f(x, y) 的方法句柄。
...
## 实战
实践环节，我们来测量一下方法句柄的性能。你可以尝试通过重构代码，将方法句柄变成常量，来提升方法句柄调用的性能。
```java
public class Foo {
...
#### [上一篇：7.  JVM如何实现反射？](7.how-jvm-implement-reflection.md)
#### [下一篇：9.  JVM如何实现invokedynamic（下）？](9.how-jvm-implement-invokedynamic.md)
